@extends('layouts.mTabs',['index'=>'Gears'])
<!-- Nav tabs -->
@section('tab')
    ...
    <div class="row">
        ...
        <div class="col-xs-12 col-sm-12 col-md-3 col-lg-3 cms_module_list">
            ...
            <ul class="list-unstyled menuList" id="components-list">
                ...
                <li class=@if(!$key)"active"@endif>
                    <a href="?={!! $template->slug !!}"  rel="tab" data-slug="{!! $template->slug !!}" class="tpl-left-items">
            ...
            <div class="templates-list  m-t-20 m-b-10">
                <div class="row m-b-10">
                    ...
                    <div class="raw tpl-list">
                        @if(count($templates))
                            @include('Gears::main_body.variations',['variations'=>$templates[0]->variations()])
                            @endif
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $("a[rel='tab']").click(function(e){
                e.preventDefault();
                var main_type = $(this).attr('data-slug');
                var pageurl = $(this).attr('href');
                $('.tpl-left-items').parent().removeClass('active');
                var general_type = $(this).attr('type');

                if(general_type){
                    $('*[data-slug="'+general_type+'"]').parent().addClass('active');
                    $('*[data-slug="'+ main_type +'"][type="'+general_type+'"]').parent().addClass('active');
                }else{
                    $('*[data-slug="'+ main_type +'"]').parent().addClass('active');
                }

                $.ajax({
                    url: '/admin/frontend/gears/body-templates-variations',
                    data: {
                        slug: main_type,
                    },headers: {
                        'X-CSRF-TOKEN':$("input[name='_token']").val()
